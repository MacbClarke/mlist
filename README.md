...
- 目录浏览与文件列表展示
- 多选与流式 ZIP 打包批量下载
  - 支持跨文件/跨目录多选，底部浮动操作栏一键打包下载
  - 目录右键菜单支持整目录“打包下载 (ZIP)”
  - 后端采用异步流式压缩（`async_zip` + Tokio Duplex 管道），**无临时文件落地，恒定 64KB 内存占用**
- 文件与目录收藏（支持快速切换至“仅看收藏”视图）
- 文件即时搜索与防抖过滤
- 多字段三态排序（名称、大小、修改时间）与列表分页浏览
- 前端请求与状态缓存（TanStack Query 实现秒开目录与平滑后台重校验）
- 常见文件在线预览
  - 图片
  - 音频
  - 视频
...
- 管理员可查看资源访问审计、用户流量统计和文件流量汇总

## 批量下载与流式 ZIP 打包

- 支持多选任意数量的文件或子目录进行批量打包下载。
- **流式打包机制**：
  - 前端请求后生成签名批量下载链接（`/api/batch-download?token=...`）。
  - 后端使用 Tokio 异步管道边读取文件、边压缩、边向客户端 HTTP 流式传输，不占用任何临时磁盘空间，内存开销恒定在 64KB 级别。
  - 客户端连接中断后自动中止压缩任务与文件读取，无后台僵尸进程。
- **目录下载**：右键点击任意目录，即可选择“打包下载 (ZIP)”直接下载整目录归档。
- **流量与审计**：打包传输的字节数实时统计至用户流量报表与资源访问审计中。
...
- `/d/...` 是单文件直接拉流入口，浏览器内访问使用登录会话 cookie 鉴权。
- 复制链接时，前端会生成 `/d/...?token=...` 签名播放链接，默认 7 天有效，适合 mpv 等不带浏览器 cookie 的播放器。
- 批量下载使用 `/api/batch-download?token=...` 签名下载链接。
- 签名链接绑定具体资源和生成用户，实际发送流量会计入该用户。
...
- 登录有失败限速，降低暴力猜解风险
- 会话、用户、审计、流量统计和文件状态使用 SQLite 持久化
